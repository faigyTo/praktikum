...
export default class sma{
	...
	execute(data,params){
		...
		if (upOrDown=="up"&&todayValue>todaySma&&(todayValue-todaySma)/todayValue>smaParamsJson.precent){
			console.log("find", {type:`sma_${upOrDown}_${length}`,value:todayValue,volume:todayCandle.Volume});
		}
		else if (upOrDown=="down"&&todaySma>todayValue&&(todaySma-todayValue)/todaySma>smaParamsJson.precent)
			console.log( "find",{type:`sma_${upOrDown}_${length}`,value:todayValue,volume:todayCandle.Volume});
		else{
